...
Next.js의 데이터 흐름은 단방향, 즉 Parents에서 child component의 방향으로 Props의 흐름이 이루어짐  
따라서 계층 구조가 복잡해지면 프롭스 드릴링 문제가 발생함  
프롭스 드릴링은 여러 컴포넌트를 지나 프롭스가 전달 되면서 발생하는 문제다

프롭스 드릴링은 다음과 같은 문제를 발생시킬 수 있음

1. 도중에 있는 컴포넌트에 불필요한 프롭스 전달
2. 타겟 컴포넌트까지 프롭스가 전달되지 않을 경우 원인 규명이 어려움
3. 필요 이상으로 코드가 복잡해짐  
   이런 문제를 해결하기 위해 프롭스를 전역으로 사용함  
   Context API, Redux 등을 사용해 Next.js에서 프롭스를 전역으로 사용함

호출 전에는 상호간 계층구조를 갖지 않음  
어느 쪽이든 호줄한다면 호출한 쪽이 부모 호출받은 쪽은 자식이 된다  
이건 컴포넌트간이던, 페이지와 컴포넌트 간이던 모두 적용됨  
한 번 관계가 성립되면 자식은 부모를 호출할 수 없음

<<<<<<< HEAD
## use client

use client를 사용하는 이유는 서버 컴포넌트와 클라이언트 컴포넌트를 구분하기 위해서 임  
Next.js는 기본적으로 서버에서 렌더링하도록 설계되어 클라이언트에서만 필요한 컴포넌트를 지정해야 할 필요가 있음  
use client를 컴포넌트 상단에 선언하면 해당 컴포넌트는 클아이언트에서만 렌더링되며 주로 상태 관리나 브라우저 전용 API사용이 필요한 경우에 사용됨

## Context API

React에서 기본적으로 제공하는 상태관리 도구 , 외부 라이브러리 설치 없이 사용 가능  
주로 전역 상태를 관리하는데 사용됨  
React.createContext()로 생선한 Context 객채와 Provider 컴포넌트를 사용해 하위 컴포넌트에 전달
...
store , reducer, action 등의 개념으로 state와 state dispatch를 관리
...
=======
>>>>>>> 1a1661a30b50a53d7290a51244737b1738594fa9
# 11/13 수업내용

## UI 라이브러리

UI 라이브러리 프레임어크 유틸리티 등의 기능이 필수는 아니나, 생산성 향상과 UI일관성 유지에는 도움 될 수 있다

## Chakra Ui

오픈소스 컴포넌트 라이브러리로 모듈화 되어 있고 접근성이 높고 보기 좋은 UI를 만들 수 있다  
버튼 모달 입력 등 다양한 내장 컴포넌트 지원  
다크모드 라이트모드 지원  
useColorMode 훅을 사용해 현재 적용중인 컬러 모드 확인 가능  
기본 컴포넌트를 조합해 새 컴포넌트를 만들기 편함  
타입스크립트 기반임

## TailwindCSS

다른 프레임워크와 다르게 CSS 규칙만 제공  
모듈이나 컴포넌트를 지원하지 않아, 필요한 경우 직접 만들어야 함  
디자인 자유도가 높음  
다크모드 라이트모드 지원  
빌드 시점에 사용하지 않는 클래스 삭제로 인해 최적화 정도가 높음  
CSS 클래스 접두사를 활용해 모바일 데스크톱 태블립 화면에서 원하는 규칙 지정 가능
...
Styled JSX 는 css in js 라이브러리다. 내장 모듈이여서 설치할 필요가 없다.
즉, css 속성 지정을 위해 자바스크립트를 사용할 수 있는 라이브러리다

## css in js 의 단점

IDE나 코드 편집기 등 개발 도구에 대한 지원이 부족함  
문법 하이라이팅 , 자동완성 , 린트 기능을 제공하지 않음  
코드 내에서 css에 대한 의존성이 점점 커지기 때문에 앱 번들도 커지고 느려짐  
서버에 미리 css를 생성해도 클라이언트에서 리액트 하이드레이션이 끝나면 css를 다시 생성해야함  
이 때문에 실행 시점에 부화가 커지며, 웹 앱이 계속 느려지게 됨 기능이 늘어날 수록 이런 현상이 심해짐

## CSS Module

css in js의 단점을 회피하기 위한 좋은 방법은 CSS Module 이다
.module.css로 끝나는 파일에서 css 클래스를 가져온다
home.module.css 파일은 일반적인 css 파일이지만 CSS 모듈이 그 내용을
자바스크립트 객체로 변환한다

클래스들은 컴포넌트 스코프를 가진다
생성된 HTML 페이지 소스를 보면 클래스 이름이 바뀌어 있는 것을 확인할 수 있다  
Styled JSX 때와 마찬가지로 이런 고유한 이름 때문에 다른 파일이라면 같은 클래스 명을 사용해도 충돌이 일어나지 않는다
만일 전역 css를 선언하고 싶다면 styles/globals.css를 만들고 사용한다  
파일명은 반드시 globals가 아니여도 가능하지만 암묵적 합의라 지키는게 좋다

ㅇㅇ

## SASS

Next에서 기본으로 지원하는 전 처리기이다  
단 패키지 설치가 필요하다  
SASS 및 SCSS 문법으로 CSS모듈을 만들고 사용할 수 있다
...
내장 API : 브라우저에 내장되어 별도의 설치가 필요 없음  
...
에러 처리 복잡성 : HTTP 오류 코드에 대한 처리가 약간 더 복잡할 수 있다. 기본적으로 fetch는 네트워크 오류만 reject함

## Axios 사용하기

axios.get()을 통해 받아온 응답 객체인 res는 단순히 JSON 데이터만 담고 있는 것이 아닌 HTTP 통신과 관련된 여려 정보를 포함하고 있다  
예를들어
res.status : HTTP 응답 상태 코드  
res.headers : 서버로 받은 헤더 정보  
res.config : 요청에 대한 설정 정보  
res.statusText : HTTP 응답 상태에 대한 설명  
res.data : 서버가 실제로 전송한 데이터  
따라서 res.data.는 서버가 전송한 실제 데이터에 접근하는 속성이다

그런데 위의 코드는 비동기 데이터 로딩과 상태 관리가 고려되지 않기 때문에 몇가지 문제가 있을 수 있다  
특히, Next.js와 같은 리액트 기반 앱에서 비동기 데이터를 처리할 때 렌더링 주기에 맞게 상태를 관리해야 한다

### 개선할 부분

1. useState , useEffect 사용 : 비동기 데이터를 가져오는 작업은 컴포넌트의 상태로 관리하는 것이 일반적. 현재 코드에서는 데이터가 비동기적으로 로드되는데 이를 관리하는 useState , useEffect 훅이 없음 ,데이터를 로드하기 전에 컴포넌트가 렌더링되어 users변수가 초기에 없어 undefinded 에러 발생 가능성 존재
2. 로딩 상태 처리  
   데이터를 불러오는 동안 로딩 상태를 추가하는 것이 좋음
